...
from schemas.rakumart_purchase_order import PurchaseOrder, PurchaseOrderCreate, PurchaseOrderPutRequest
models.Base.metadata.create_all(bind=engine)

router = APIRouter(
    prefix="/rakumart/purchase",
    tags=["rakumart purchase order"],
)
...
@router.post("/order", response_model=PurchaseOrder)
async def create_rakumart_purchase_order(purchase_order: PurchaseOrderCreate, db: Session = Depends(get_db)):
